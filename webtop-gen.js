...
function logResolvedOptions(options, overrides) {
    ...
    console.log(`  updates: ${DEFAULTS.updates} -> ${options.updates}${overrides.has('updates') ? ' (override)' : ''}`);
    console.log(
        `  interval-ms: ${DEFAULTS.intervalMs} -> ${options.intervalMs}${overrides.has('interval-ms') ? ' (override)' : ''}`
    );
    console.log(`  iface: auto -> ${ifaceDisplay}${overrides.has('iface') ? ' (override)' : ''}`);
    console.log(
    ...
    console.log(`  out: ${options.out}${overrides.has('out') ? ' (override)' : ''}`);
}
...
function parseLoadAvg(raw) {
    ...
    return [round2(Number(one) || 0), round2(Number(five) || 0), round2(Number(fifteen) || 0)];
}
...
function parseNetDev(raw) {
    ...
    for (const line of lines) {
        ...
        const nums = rest.trim().split(/\s+/).map((value) => Number(value) || 0);
...
function selectNetInterface(netDevMap, preferredIface, defaultRouteIface) {
    ...
    if (defaultRouteIface && netDevMap.has(defaultRouteIface)) return defaultRouteIface;
...
function parseSensorsOutput(raw) {
    ...
    for (const line of lines) {
        ...
        const measureMatch = /^\s*([^:]+):\s*\+?(-?\d+(?:\.\d+)?)\s*°?C\b/i.exec(line);
...
function selectDiskTempFromSensors(entries, preferredId) {
    ...
    const diskCandidates = entries
        .filter((entry) => {
            const chip = entry.chip.toLowerCase();
            const label = entry.label.toLowerCase();
            return /nvme|drivetemp|ssd|ata/.test(chip) && /(composite|temp1|temperature)/.test(label);
        })
        .sort((a, b) => b.celsius - a.celsius);
...
async function createCpuTempReader(preferredCpuTempId) {
    ...
    try {
        ...
        const thermalEntries = await readdir(thermalBase, { withFileTypes: true });
        for (const entry of thermalEntries) {
            ...
            let type = '';
            try {
                type = (await readFile(path.join(zonePath, 'type'), 'utf8')).trim();
            } catch {
    ...
    try {
        ...
        for (const hwmonEntry of hwmonEntries) {
            ...
            try {
                chipName = (await readFile(path.join(dirPath, 'name'), 'utf8')).trim() || chipName;
            } catch {
    ...
    const cpuHintCount = probes.filter((probe) => looksLikeCpuTemp(probe.name, probe.label)).length;
    ...
    async function read() {
        if (preferredCpuTempId) {
            ...
            if (sensorsRaw) {
                ...
                const selected = selectCpuTempFromSensors(sensors.entries, preferredCpuTempId);
                if (selected) {
                    return selected;
                }
                return {
        ...
        const cpuCandidates = candidates.filter((candidate) => candidate.score > 0);
...
async function readDiskRows(maxDisks) {
    ...
    let stdout = '';
    try {
        ({ stdout } = await execFileAsync('df', args, { encoding: 'utf8', maxBuffer: 1024 * 1024 }));
    } catch {
        return {
            rows: [],
            skippedEfi: 0
    ...
    for (const line of lines) {
        ...
        if (fsName.startsWith('tmpfs') || fsName.startsWith('devtmpfs')) continue;
...
async function readStaticSnapshot(maxDisks, preferredDiskTempId) {
    ...
    const sensors = parseSensorsOutput(sensorsRaw);
    const selectedDiskTemp = selectDiskTempFromSensors(sensors.entries, preferredDiskTempId);
...
async function collectUpdates(updateCount, intervalMs, preferredIface, cpuTempReader) {
    let previous = await readDynamicSnapshot(preferredIface, cpuTempReader);
    const updates = [];

    for (let i = 0; i < updateCount; i += 1) {
        await sleep(intervalMs);
        const current = await readDynamicSnapshot(previous.iface || preferredIface, cpuTempReader);

        const deltaSec = Math.max(0.001, (current.timestampMs - previous.timestampMs) / 1000);
        const threadKeys = [...current.cpuStats.keys()]
            .filter((key) => /^cpu\d+$/.test(key))
            .sort((a, b) => Number(a.slice(3)) - Number(b.slice(3)));

        const perThreadPct = threadKeys.map((key) => computeCpuUsagePct(previous.cpuStats, current.cpuStats, key));
        const totalUsagePct = computeCpuUsagePct(previous.cpuStats, current.cpuStats, 'cpu');

        const downloadKibps = round2(Math.max(0, (current.rxBytes - previous.rxBytes) / deltaSec / 1024));
        const uploadKibps = round2(Math.max(0, (current.txBytes - previous.txBytes) / deltaSec / 1024));
...
async function main() {
    ...
    if (options.cpuTempId) {
        console.log(`CPU temp selection: forcing sensors id match for "${options.cpuTempId}"`);
    }

    console.log('Reading static snapshot...');
    const staticSnapshot = await readStaticSnapshot(options.maxDisks, options.diskTempId);
    console.log(
    ...
    if (options.diskTempId) {
        console.log(`Disk temp selection: forcing sensors id match for "${options.diskTempId}"`);
    }
    if (staticSnapshot.diskTempC != null) {
        ...
        console.log(`Disk temp unresolved (source=${staticSnapshot.diskTempSource}).`);
    }

    console.log(`Capturing ${options.updates} updates every ${options.intervalMs}ms...`);
    ...
    if (cpuTempReader.sensorsAvailable()) {
        console.log('CPU temp source: using sensors fallback when sysfs probes are unavailable.');
    }
